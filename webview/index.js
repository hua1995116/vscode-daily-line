(function () {
  ...
  const innerWrapper = document.getElementById("snippet-inner");
  ...
  function getCodeHtml(line, time, day) {
    let timeHTML = '';
    if (time < 1000 * 60 * 60) {
      timeHTML = Math.fround(time / 1000 / 60).toFixed(2) + 'min';
    } else {
      const hours = time / 1000 / 60 / 60;
      const timeradio = ~~((hours * 100) / 24);
      timeobj = make_bar(timeradio, bar_styles[0], 5, 20);
      timeHTML = `${timeobj.str} ${timeradio}%`;
    }
    ...
    return `<div id="day">${day}</div><div>💻 Today coding ${line} lines</div><div>⏱️ ${timeHTML}</div><div class="line"></div>`;
  }

  window.addEventListener("message", (e) => {
    if (e) {
      if (e.data.type === "update") {
        const { fontFamily, bgColor, line, time, day, quesNum, question } = e.data;
        console.log(e.data);
        let defaulHtml = getCodeHtml(line, time, day);
        if (quesNum) {
          const qr = document.getElementById("qr");
          qr.style.display = 'block';
          const qrcode = new QRCode(qr, {
            text: `https://github.com/hua1995116/daily-questions/issues/${quesNum}`,
            width: 60,
            height: 60,
            colorDark : "#000000",
            colorLight : "#ffffff",
          ...
          defaulHtml += `<div>❓ ${question}</div>`;
        }
        innerWrapper.innerHTML = defaulHtml;
      }
    }
  });
})();
